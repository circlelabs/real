...
std::map<int, std::queue<struct real::waypoint> > mapOfPaths;
...
*/
bool saveFlightData(real::SaveFlightData::Request &req, real::SaveFlightData::Response &res)
{
	isMonitoringTelemetry = false;
	ROS_INFO("Saving to file %s...", req.filename.c_str());
	
	FILE *fp = fopen((ros::package::getPath("real")+"/flightData/"+req.filename).c_str(), "w");
	if(fp != NULL)
	{
		...
		//here goes the magic
		std::map<int, std::queue<struct real::waypoint> >::iterator ii;
