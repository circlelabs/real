...
#include "real/RequestPlaneID.h"
#include "real/GoToWaypoint.h"
#include "real/PlaneCoordinator.h"
#include "real/LoadPath.h"
#include "real/RequestWaypointInfo.h"
#include "real/LoadCourse.h"
...
std::map<int, real::PlaneCoordinator> planesArray;
...
bool requestPlaneID(real::RequestPlaneID::Request &req, real::RequestPlaneID::Response &res)
{
	...
	if(req.requestedID == -1)
	{
		...
		while(true)
		{
			...
			else
			{
				...
				planesArray[id] = real::PlaneCoordinator();
	...
	else
	{
		...
		{
			planesArray[req.requestedID] = real::PlaneCoordinator();
...
*/
bool goToWaypoint(real::GoToWaypoint::Request &req, real::GoToWaypoint::Response &res)
{
	...
	{
		//construct waypoint
		struct real::waypoint pointFromService;
		...
		if(planesArray[req.planeID].goToPoint(pointFromService, req.isAvoidanceManeuver, req.isNewQueue))
		{
			...
			*/
			...
			{
				//get the command
				real::Command commandToSend = planesArray[req.planeID].getPriorityCommand();
...
*/
bool loadPathCallback(real::LoadPath::Request &req, real::LoadPath::Response &res)
{
	...
	if(isValidPlaneID(req.planeID))
	{
		//open our file
		FILE *fp;
		fp = fopen((ros::package::getPath("real")+"/paths/"+req.filename).c_str(), "r");
...
*/
bool loadCourseCallback(real::LoadCourse::Request &req, real::LoadCourse::Response &res)
{
	ROS_INFO("Received request: Load course from \"%s\"\n", req.filename.c_str());
	
	//open our file
	FILE *fp;
	fp = fopen((ros::package::getPath("real")+"/courses/"+req.filename).c_str(), "r");
...
*/
bool requestWaypointInfoCallback(real::RequestWaypointInfo::Request &req, real::RequestWaypointInfo::Response &res)
{
	//check that the request ID is valid
	if(isValidPlaneID(req.planeID))
	{
		//fill out the waypoint to return
		real::waypoint temp = planesArray[req.planeID].getWaypointOfQueue(req.isAvoidanceWaypoint, req.positionInQueue);
...
int main(int argc, char **argv)
{
	...
	commandPub = n.advertise<real::Command>("commands", 1000);
