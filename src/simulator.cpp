...
std::map<int, real::SimulatedPlane> simPlaneMap;
...
*/
void commandCallback(const real::Command::ConstPtr& msg)
{
...
up a course as well.
*/
bool createSimulatedPlaneCallback(real::CreateSimulatedPlane::Request &req, real::CreateSimulatedPlane::Response &res)
{
	//create a service to get a plane ID to use
	real::RequestPlaneID srv;
	...
	if(requestPlaneIDClient.call(srv))
	{
		...
		//real::SimulatedPlane newPlane(srv.response.planeID, req);
		simPlaneMap[srv.response.planeID] = real::SimulatedPlane(srv.response.planeID, req);
...
*/
bool deleteSimulatedPlaneCallback(real::DeleteSimulatedPlane::Request &req, real::DeleteSimulatedPlane::Response &res)
{
...
int main(int argc, char **argv)
{
	...
	ros::Publisher telemetryPub = n.advertise<real::TelemetryUpdate>("telemetry", 1000);
	...
	requestPlaneIDClient = n.serviceClient<real::RequestPlaneID>("request_plane_ID");
	...
	while(ros::ok())
	{
		...
		if(loopPosition == 0)
		{
			...
			real::TelemetryUpdate tUpdate;
			std::map<int, real::SimulatedPlane>::iterator ii;
