...
#include "real/DeleteSimulatedPlane.h"
#include "real/SaveFlightData.h"

//USER DEFINED EVALUATION SETTINGS
#define TIME_LIMIT 150 //10 minutes
...
std::map<int, real::TelemetryUpdate> previousUpdatesMap;
std::map<int, real::TelemetryUpdate> latestUpdatesMap;

//the list of waypoints to go at
std::map<int, std::queue<real::waypoint> > waypointQueues;
...
void endEvaluation()
{
	...
	FILE *fp;
	fp = fopen((ros::package::getPath("real")+"/scores/"+scoresheetFilename+".score").c_str(), "w");
	...
	else
	{
		...
		if(totalMinDist != 0) fprintf(fp, "%lf\n", totalMinDist/totalDistTraveled);
...
bool createCourseUAVs(std::string filename)
{
	//open our file
	FILE *fp;
	fp = fopen((ros::package::getPath("real")+"/courses/"+filename).c_str(), "r");
		...
		while(fgets(buffer, sizeof(buffer), fp))
		{
			...
			else
			{
				...
				if(isFirstPoint.find(planeID) == isFirstPoint.end())
				{
					...
					latestUpdatesMap[planeID] = real::TelemetryUpdate();
					...
					previousUpdatesMap[planeID] = real::TelemetryUpdate();
...
void telemetryCallback(const real::TelemetryUpdate::ConstPtr& msg)
{
	...
	if(msg->planeID == maxAlivePlane)
	{
		...
		struct real::waypoint current, other;
		...
		for(int id = 0; id <= lastPlaneID; id++)
		{
			...
			while(!waypointQueues[id].empty() && distanceBetween(waypointQueues[id].front(), current) < COLLISION_THRESHOLD)
			{
				...
				struct real::waypoint temp = waypointQueues[id].front();
...
int main(int argc, char **argv)
{
	...
	//set up ROS services
	real::LoadCourse srv;
	createSimulatedPlaneClient = n.serviceClient<real::CreateSimulatedPlane>("create_simulated_plane");
	deleteSimulatedPlaneClient = n.serviceClient<real::DeleteSimulatedPlane>("delete_simulated_plane");
	loadCourseClient = n.serviceClient<real::LoadCourse>("load_course");
	saveFlightDataClient = n.serviceClient<real::SaveFlightData>("save_flight_data");
